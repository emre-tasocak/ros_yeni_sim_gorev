"""
3-Wheeled Omni Robot Kinematics

Physical wheel configuration (top view, x=forward, y=left):
  w1: Right-front, α₁ = -60°
  w2: Left-front,  α₂ = +60°
  w3: Rear,        α₃ = 180°

Omni wheel velocity formula: v_i = -vx·sin(αᵢ) + vy·cos(αᵢ) + L·ω

Inverse kinematics matrix (J_inv): [w1, w2, w3] = J_inv @ [vx, vy, omega]
  J_inv = [[ √3/2,  1/2, L ],   # w1 (α=-60°)
           [-√3/2,  1/2, L ],   # w2 (α=+60°)
           [  0,   -1,   L ]]   # w3 (α=180°)

Forward kinematics matrix (J): [vx, vy, omega] = J @ [w1_dist, w2_dist, w3_dist]
  J = inv(J_inv) — computed via numpy
"""

import numpy as np
import math


class OmniKinematics:
    """3-wheeled omni robot kinematics class."""

    def __init__(self, wheel_radius: float, wheel_base: float,
                 ticks_per_rev: int, gear_ratio: float = 1.0):
        """
        Args:
            wheel_radius:  Wheel radius (meters)
            wheel_base:    Distance from center to wheel (meters)
            ticks_per_rev: Encoder ticks per wheel revolution
            gear_ratio:    Gear ratio
        """
        self.r = wheel_radius
        self.L = wheel_base
        self.ticks_per_rev = ticks_per_rev
        self.gear_ratio = gear_ratio

        # Distance traveled per encoder tick (meters)
        self.dist_per_tick = (2.0 * math.pi * self.r) / self.ticks_per_rev

        # Inverse kinematics matrix: robot body velocity → wheel linear velocity
        # v_i = -vx·sin(αᵢ) + vy·cos(αᵢ) + L·ω
        L = self.L
        s = math.sqrt(3) / 2.0  # sin(60°) = √3/2
        self.J_inv = np.array([
            [ s,   0.5,  L],   # w1: right-front, α₁ = -60°
            [-s,   0.5,  L],   # w2: left-front,  α₂ = +60°
            [ 0.0, -1.0, L],   # w3: rear,        α₃ = 180°
        ])

        # Forward kinematics matrix: wheel displacements → robot displacement
        self.J = np.linalg.inv(self.J_inv)

    # ------------------------------------------------------------------
    # Velocity conversions
    # ------------------------------------------------------------------

    def robot_vel_to_wheel_vel(self, vx: float, vy: float, omega: float):
        """
        Converts robot body velocities to wheel linear velocities (m/s).

        Returns:
            (w1, w2, w3): Speed in m/s for each wheel
        """
        robot_vel = np.array([vx, vy, omega])
        wheel_vels = self.J_inv @ robot_vel
        return float(wheel_vels[0]), float(wheel_vels[1]), float(wheel_vels[2])

    def wheel_vel_to_robot_vel(self, w1: float, w2: float, w3: float):
        """
        Converts wheel velocities to robot body velocities.

        Returns:
            (vx, vy, omega)
        """
        wheel_vels = np.array([w1, w2, w3])
        robot_vel = self.J @ wheel_vels
        return float(robot_vel[0]), float(robot_vel[1]), float(robot_vel[2])

    # ------------------------------------------------------------------
    # Encoder conversions
    # ------------------------------------------------------------------

    def velocity_to_ticks_per_sec(self, velocity_ms: float) -> float:
        """Converts wheel linear velocity (m/s) to encoder ticks/sec."""
        return velocity_ms / self.dist_per_tick

    def ticks_per_sec_to_velocity(self, ticks_per_sec: float) -> float:
        """Converts encoder ticks/sec to wheel linear velocity (m/s)."""
        return ticks_per_sec * self.dist_per_tick

    def ticks_to_distance(self, ticks: int) -> float:
        """Converts encoder tick count to wheel travel distance (meters)."""
        return ticks * self.dist_per_tick

    # ------------------------------------------------------------------
    # Odometry computation
    # ------------------------------------------------------------------

    def compute_odometry(self,
                         delta_ticks_1: int,
                         delta_ticks_2: int,
                         delta_ticks_3: int,
                         current_x: float,
                         current_y: float,
                         current_yaw: float):
        """
        Computes new robot pose from encoder delta values.

        Args:
            delta_ticks_*: Encoder change for each wheel (ticks)
            current_*:     Current robot pose

        Returns:
            (new_x, new_y, new_yaw, dx_local, dy_local, dyaw)
            dx_local, dy_local: Local displacement in robot frame
        """
        # Ticks → distance
        dp1 = self.ticks_to_distance(delta_ticks_1)
        dp2 = self.ticks_to_distance(delta_ticks_2)
        dp3 = self.ticks_to_distance(delta_ticks_3)

        # Forward kinematics: local displacement
        displacement = self.J @ np.array([dp1, dp2, dp3])
        dx_local = float(displacement[0])
        dy_local = float(displacement[1])
        dyaw = float(displacement[2])

        # Transform local displacement to world frame (using mid-yaw angle)
        mid_yaw = current_yaw + dyaw / 2.0
        cos_y = math.cos(mid_yaw)
        sin_y = math.sin(mid_yaw)

        dx_world = dx_local * cos_y - dy_local * sin_y
        dy_world = dx_local * sin_y + dy_local * cos_y

        new_x = current_x + dx_world
        new_y = current_y + dy_world
        new_yaw = current_yaw + dyaw

        # Normalize angle to [-pi, pi]
        new_yaw = math.atan2(math.sin(new_yaw), math.cos(new_yaw))

        return new_x, new_y, new_yaw, dx_local, dy_local, dyaw

    # ------------------------------------------------------------------
    # Utility functions
    # ------------------------------------------------------------------

    @staticmethod
    def normalize_angle(angle: float) -> float:
        """Normalizes angle to [-pi, pi]."""
        return math.atan2(math.sin(angle), math.cos(angle))

    @staticmethod
    def distance_to_goal(x1: float, y1: float, x2: float, y2: float) -> float:
        """Euclidean distance between two points."""
        return math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2)
