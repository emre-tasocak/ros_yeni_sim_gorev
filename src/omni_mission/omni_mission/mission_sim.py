...
=============================================
GÖREV AKIŞI:
  1. WAITING   – 10 saniye bekle (Gazebo oturulsun)
...
KONTROLCÜ:
  Omni robot döndürülmez (w=0), sadece öteleme:
    vx_world = Kp * (hedef_x - x)
    vy_world = Kp * (hedef_y - y)
  Hız MAX_VEL ile sınırlandırılır.
  Hedefe yaklaşınca APPROACH_VEL'e indirilir.
"""
...
class OmniMissionSim(Node):
    ...
    MAX_VEL       = 0.22   # Maksimum öteleme hızı [m/s]
    APPROACH_DIST = 0.80   # Bu mesafeden itibaren yavaşla [m]
    APPROACH_VEL  = 0.07   # Yaklaşma minimum hızı [m/s]
    KP            = 1.2    # Konum P katsayısı
    ...
    def __init__(self):
        ...
        self._dt = 0.05   # 20 Hz kontrol döngüsü
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # WAITING – Gazebo oturulana kadar bekle
    # ══════════════════════════════════════════════════════════════════════════
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # SCANNING – Robot tamamen durur, sadece LiDAR çalışır
    # ══════════════════════════════════════════════════════════════════════════

    def _st_scanning(self):
        self._stop()   # tarama boyunca kesinlikle hareketsiz

        if self.scan is None:
            self.get_logger().warn('LiDAR bekleniyor...', throttle_duration_sec=1.0)
            return

        # Tarama süresini bekle
        ...
        # ── Tarama bitti: ön ±SCAN_HALF_DEG içinde en yakın noktayı bul ──
        # range_min=0.28m → robot gövdesi (0.13m) ve tekerleri (0.20m) görünmez
        ...
        if math.isinf(best_r):
            self.get_logger().warn(
                'Ön yarımkürede cisim bulunamadı, yeniden taranıyor...',
        ...
        self.get_logger().info(
            ...
            f'   Cisim (dünya)  : ({obs_x:.2f}, {obs_y:.2f})\n'
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # NAVIGATING / RETURNING – Hedefe düz git, dönme yok
    # ══════════════════════════════════════════════════════════════════════════

    def _st_nav(self, gx: float, gy: float, next_state: State):
        """
        Saf P kontrolcü ile hedefe git.
        Omni robot döndürülmez (w = 0) – sadece öteleme komutu verilir.

        Adımlar:
          1. Hata vektörünü hesapla  (ex, ey) = (gx - x, gy - y)
          2. Hıza çevir: vx = Kp*ex, vy = Kp*ey
          3. MAX_VEL ile sınırla
          4. Yaklaşma bölgesinde (dist < APPROACH_DIST) hızı lineer azalt
          5. Dünya → gövde çerçevesine döndür
          6. Yayımla (w = 0)
        """
        ex = gx - self.x
        ey = gy - self.y
        ...
        # Yaklaşma bölgesi: hızı lineer olarak APPROACH_VEL'e indir
        if dist < self.APPROACH_DIST:
            ratio = dist / self.APPROACH_DIST          # 0..1
        ...
        # Omni robot herhangi bir yöne bakıyor olabilir;
        # hız vektörünü gövde eksenlerine dönüştür.
        c = math.cos(self.yaw)
        s = math.sin(self.yaw)
        vx_b =  c * vx_w + s * vy_w
        vy_b = -s * vx_w + c * vy_w

        # ── Yayımla (w = 0 → dönme yok) ──────────────────────────────────
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # AT_GOAL – 3 saniye bekle, sonra eve dön
    # ══════════════════════════════════════════════════════════════════════════
    ...
    def _st_at_goal(self):
        ...
        if kalan > 0:
            self.get_logger().info(
                f'Hedefe ulaşıldı. Eve dönüşe {kalan:.1f}s kaldı...',
                throttle_duration_sec=1.0
            )
        else:
            self.get_logger().info(
                f'Eve dönüş başlıyor → ({self.home_x:.2f}, {self.home_y:.2f})'
            )
